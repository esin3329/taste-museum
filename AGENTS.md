...
- The production museum entry intentionally presents a responsive full-viewport web surface: app-scoped styles hide PhoneFrame chrome, status indicators, and the simulated keyboard while keeping the runtime providers and interaction behavior mounted. Runtime fixtures continue to render the calibrated phone frame for mobile QA. Do not remove the web-view overrides or replace the production entry with a device mockup.
