...
## Boundaries
The app has no login and is intended for one person behind Tailscale. In server mode, SQLite stores metadata and the mini PC stores uploaded originals under the configured data directory. Music demo is explicitly a synthetic sample, not a recording of a real song. User-entered links accept only HTTP(S). Persisted items can be edited or deleted, and the SQLite database plus uploads can be backed up and restored with the scripts in `scripts/`; the optional systemd timer keeps the latest 14 backups. Room creation/editing remains outside the fixed four-room prototype, and HEIC thumbnail conversion is still future work.
